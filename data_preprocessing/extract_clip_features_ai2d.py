#!/usr/bin/env python3
"""
Extract CLIP features for all images in AI2D dataset with multiprocessing support.
Only extracts features for images that appear in the train.jsonl file.
"""
...
def extract_features_from_json(
    ...
    Only processes images that appear in the JSONL file.
    
    Args:
        json_file: Path to AI2D JSON/JSONL file
        images_dir: Directory containing AI2D images
    ...
    # Only include images that appear in the JSONL file
    image_items = []
    seen = set()
    for idx, entry in enumerate(data):
        image_path = entry.get('image', '')
        if image_path:
            # Extract filename from path (handles both full paths and relative paths)
            # e.g., "/media/data2/maschan/internvl/data/ai2diagram/images/10.png" -> "10.png"
            # or "data/ai2diagram/images/10.png" -> "10.png"
...
def main():
    parser = argparse.ArgumentParser(description="Extract CLIP features for AI2D images")
    parser.add_argument(
        "--json_file",
        type=str,
        default="/media/data2/maschan/internvl/data/ai2diagram/ai2d_train.jsonl",
        help="Path to AI2D JSON/JSONL file"
    )
    parser.add_argument(
        "--images_dir",
        type=str,
        default="/media/data2/maschan/internvl/data/ai2diagram/images",
        help="Directory containing AI2D images"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/home/maschan/ddfm/InternVL/clustering/ai2d",
